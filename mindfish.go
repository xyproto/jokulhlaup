...
func ServeEngines(userState *genericsite.UserState, mainMenuEntries genericsite.MenuEntries) {
	...
	userEngine.ServePages("mindfish.me")

	// The admin engine
	adminEngine := siteengines.NewAdminEngine(userState)
	adminEngine.ServePages(MindfishBaseCP, mainMenuEntries)

	// Wiki engine
	wikiEngine := siteengines.NewWikiEngine(userState)
	wikiEngine.ServePages(MindfishBaseCP, mainMenuEntries)
}
...
func main() {
	...
	mainMenuEntries := ServeMindfish(userState, "/js/jquery-"+JQUERY_VERSION+".min.js")
